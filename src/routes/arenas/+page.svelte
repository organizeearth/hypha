<script>
  import OrganicText from "$lib/organicText.svelte";
  import CollabGraph from "$lib/CollabGraph.svelte";
  import EntityChip from "$lib/entityChip.svelte";
  import EntityFlower from "$lib/entityFlower.svelte";
  import EntityHeading from "$lib/entityHeading.svelte";
  import EntityLegend from "$lib/entityLegend.svelte";
  import { questions } from "$lib/demo/demo";
  import { myScaledColor } from "$lib/colors";
  /** @type {import('./$types').PageData} */
  export let data;

  $: wegotnets = data && typeof data.nets !== "undefined";

  $: arenas = data.arenas;

  const borderColorStyle = (i) => {
    return `border-color: ${myScaledColor((i * 1.0) / arenas.length)}`;
  };

  const colorStyle = (i) => {
    return `${myScaledColor((i * 1.0) / arenas.length)}`;
  };

  console.log({ arenas: data.arenas });
  let activeTab = "arenas";
  let tabs = ["arenas", "networks", "legend"];
</script>

<svelte:head>
  <title>Hypha 0</title>
  <meta name="description" content="Svelte demo app" />
</svelte:head>

<main>
  <section>
    <OrganicText tagType="h1" textContent="Grassroots Alliance" />
    <EntityHeading
      textContent="A prototype tech tool for climate justice coordination networks"
      entityType="none"
    />
  </section>
  <div class="tab-list-wrapper">
    <nav class="tab-list">
      <ul>
        {#each tabs as tab}
          <li class:active={tab === activeTab}>
            <label>
              <input
                class="nav-radio"
                type="radio"
                bind:group={activeTab}
                name="activeTab"
                value={tab}
              />
              {tab}
            </label>
          </li>
        {/each}
      </ul>
    </nav>
    <div class="spacer" />
  </div>
  <section class="inner" class:hidden={activeTab != "arenas"}>
    <div class="arena-bucket">
      {#each arenas as arena, i}
        <a href="#">
          <div style={borderColorStyle(i)} class="arena">
            <div>{arena.name}</div>
            <div>
              <svg
                ><circle
                  cx="10"
                  cy="10"
                  r="5"
                  stroke-width="3"
                  fill={colorStyle(i)}
                /></svg
              >
            </div>
          </div>
        </a>
      {/each}
    </div>
  </section>
  <section class="inner" class:hidden={activeTab != "networks"}>
    {#if wegotnets}
      <OrganicText textContent="Networks" />
      <p>There is currently <strong>1</strong> network using Hypha Blue</p>
      <section class="chip-list-wrapper">
        <ul class="chip-list">
          {#each data.nets as network}
            <!-- Note: field names are case sensitive-->
            <li>
              <EntityChip
                address="/n/{network.id}"
                label={network.name}
                extra={"(3 collabs)"}
                entityType="network"
              />
            </li>
          {/each}
        </ul>
      </section>
    {:else}
      <p>No records</p>
    {/if}
  </section>
  <section class="inner" class:hidden={activeTab != "legend"}>
    <EntityLegend {data} />
  </section>
</main>

<style>
  .arena-bucket {
    display: grid;
    gap: 0.5em;
    grid-template-columns: repeat(4, 1fr);
  }
  .arena {
    height: 3em;
    border-width: 4px;
    border-radius: 1em;
    border-color: gray;
    border-style: solid;
    padding: 0.5em;
  }
</style>
